...
public class Ped_Prod_CantidaServiceDB implements  Ped_Prod_CantidaService{
	...
	public Ped_Prod_Cantida delete(long id) {
		if(repositorioPedProd.existsById(id)) {
			Ped_Prod_Cantida pedProd=repositorioPedProd.findById(id).get();
	...
	public void deleteAllByPedido (List<Ped_Prod_Cantida> uniones) {
		...
		for (int i=0; i< uniones.size();i++) {
			idUnion=uniones.get(i).getId(); /**Recoge identificador**/
			repositorioPedProd.deleteById(idUnion); /**Lo borramos del repositorio**/
		}
	}
