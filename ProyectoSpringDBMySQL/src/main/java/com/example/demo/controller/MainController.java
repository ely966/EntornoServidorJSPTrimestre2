...
//**----------------Seleccionar el pedido a editar-------------------------------**//
	...
	public String editarPedidoNuevo(@RequestParam(required=false,name=("cantidad"))Integer[] listaCantidad,@Valid @ModelAttribute("pedido") Pedido pedido , @RequestParam(required=false,name=("id"))Integer idd,Model model, ServletResponse res) throws IOException   {
		//**Comprobamos que la session y el usuario exista**//
...
//**Siguiente parte de añadir un nuevo pedido**//
	...
	//**Comprobamos que la session y el usuario exista**//
		if(session.getAttribute("userSesion") == null) {//**SI no existe**//
			...
			for (int i=0; i< productosListado.size();i++) {
				...
				Ped_Prod_Cantida pedPro= new Ped_Prod_Cantida(pedido,productosListado.get(i),listaCantidad[i]);
			...
			pedido.setUser(usuario);
			pedido.setListaProductos(productosLis);
			//**Creamos una lista de productos y una de cantidades**//
			List<Producto> productos=new ArrayList<>();
			List<Integer> cantidades=new ArrayList<>();
			//**Recorremos las uniones de pedidosYProductos y asi vamos añadiendo en la lista de productos, los productos y en la otra la cantidad en orden**//
				for (int i=0; i< listaProdYCantidad.size();i++) {
	...
	public String addNewAlaListapedidos(@RequestParam(required=false,name=("cantidad"))Integer[] listaCantidad ,@Valid @ModelAttribute("pedido") Pedido pedido, BindingResult binding) {
		if(binding.hasErrors()) { //**Sino existe**//
			return "redirect:/";
		}else {
			//**Primero creamos una lista de integer, donde recogeremos al lista de integer recogida de las cantidades seleccionada del formulario**//
			Integer[] listDeCantidades =listaCantidad;
			...
			//**Creo uniones de cada producto con el pedido**//	
			//**Recojo el ultimo pedido añadiendo en el repositorio de pedidos, que es el ultimo añadido**//
